  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    if call.method == "getPlatformVersion" { // Verifica si se está llamando el método "getPlatformVersion"
      ...
      switch centralManager?.state {
        ...
        result(FlutterError(code: "INVALID_ADDRESS", message: "Invalid UUID string", details: nil))
        return
    }
      ...
      if connectedPeripheral?.state == CBPeripheralState.connected {
        ...
        guard let arguments = call.arguments as? [Int] else {
         result(FlutterError(code: "INVALID_DATA", message: "Expected [UInt8] or [Int]", details: nil))
        return
        }
        //let bytes = arguments
        ...
        result(FlutterError(code: "INVALID_ARGS", message: "String expected", details: nil))
        return
    }
    self.stringprint = text
