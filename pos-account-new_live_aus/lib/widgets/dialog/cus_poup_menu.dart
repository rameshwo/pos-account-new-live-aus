...
const double _kMenuWidthStep = 80.0;
...
/// shows a popup menu, consider using [CusPopupMenuButton].
///
/// The type `T` is the type of the value(s) the entry represents. All the
/// entries in a given menu must represent values with consistent types.
///
/// A [CusPopupMenuEntry] may represent multiple values, for example a row with
/// several icons, or a single entry, for example a menu item with an icon (see
/// [CusPopupMenuItem]), or no value at all (for example, [PopupMenuDivider]).
///
/// See also:
///
///  * [CusPopupMenuItem], a popup menu entry for a single value.
///  * [PopupMenuDivider], a popup menu entry that is just a horizontal line.
///  * [CheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [CusPopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
abstract class CusPopupMenuEntry<T> extends StatefulWidget {
  /// Abstract const constructor. This constructor enables subclasses to provide
  /// const constructors so that they can be used in const expressions.
  const CusPopupMenuEntry({super.key});
  ...
  ///
  /// If the [CusPopupMenuEntry] represents a single value, this should return true
...
/// See also:
///
///  * [CusPopupMenuItem], for the kinds of items that this widget divides.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [CusPopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class PopupMenuDivider extends CusPopupMenuEntry<Never> {
...
// item lines up with the center of its CusPopupMenuButton.
...
///
/// Typically the [child] of a [CusPopupMenuItem] is a [Text] widget. More
/// elaborate menus with icons can use a [ListTile]. By default, a
/// [CusPopupMenuItem] is [kMinInteractiveDimension] pixels high. If you use a widget
...
///
/// ```dart
/// const CusPopupMenuItem<WhyFarther>(
...
/// {@end-tool}
///
/// See the example at [CusPopupMenuButton] for how this example could be used in a
/// complete menu, and see the example at [CheckedPopupMenuItem] for one way to
/// keep the text of [CusPopupMenuItem]s that use [Text] widgets in their [child]
/// slot aligned with the text of [CheckedPopupMenuItem]s or of [CusPopupMenuItem]
/// that use a [ListTile] in their [child] slot.
///
/// See also:
///
///  * [PopupMenuDivider], which can be used to divide items from each other.
///  * [CheckedPopupMenuItem], a variant of [CusPopupMenuItem] with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [CusPopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class CusPopupMenuItem<T> extends CusPopupMenuEntry<T> {
  /// Creates an item for a popup menu.
  ///
  /// By default, the item is [enabled].
  ///
  /// The `enabled` and `height` arguments must not be null.
  const CusPopupMenuItem({
    super.key,
    this.value,
    this.onTap,
    this.enabled = true,
  ...
  @override
  PopupMenuItemState<T, CusPopupMenuItem<T>> createState() =>
      PopupMenuItemState<T, CusPopupMenuItem<T>>();
}

/// The [State] for [CusPopupMenuItem] subclasses.
///
/// By default this implements the basic styling and layout of Material Design
/// popup menu items.
///
/// The [buildChild] method can be overridden to adjust exactly what gets placed
/// in the menu. By default it returns [CusPopupMenuItem.child].
///
/// The [handleTap] method can be overridden to adjust exactly what happens when
/// the item is tapped. By default, it uses [Navigator.pop] to return the
/// [CusPopupMenuItem.value] from the menu route.
///
/// This class takes two type arguments. The second, `W`, is the exact type of
/// the [Widget] that is using this [State]. It must be a subclass of
/// [CusPopupMenuItem]. The first, `T`, must match the type argument of that widget
/// class, and is the type of values returned from this menu.
class PopupMenuItemState<T, W extends CusPopupMenuItem<T>> extends State<W> {
  /// The menu item contents.
  ///
  /// Used by the [build] method.
  ///
  /// By default, this returns [CusPopupMenuItem.child]. Override this to put
  ...
  ///
  /// By default, uses [Navigator.pop] to return the [CusPopupMenuItem.value] from
  /// the menu route.
  @protected
  void handleTap() {
    widget.onTap?.call();
...
/// matches the default minimum height of a [CusPopupMenuItem]. The horizontal
...
///     }
///   },
///   itemBuilder: (BuildContext context) => <CusPopupMenuEntry<Commands>>[
...
///     const CusPopupMenuItem<Commands>(
...
/// See also:
///
///  * [CusPopupMenuItem], a popup menu entry for picking a command (as opposed to
///    toggling a value).
///  * [PopupMenuDivider], a popup menu entry that is just a horizontal line.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [CusPopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class CheckedPopupMenuItem<T> extends CusPopupMenuItem<T> {
...
class _PopupMenuRouteLayout extends SingleChildLayoutDelegate {
  ...
  // The index of the selected item, or null if CusPopupMenuButton.initialValue
  // was not specified.
...
class _PopupMenuRoute<T> extends PopupRoute<T> {
  ...
  final RelativeRect position;
  final List<CusPopupMenuEntry<T>> items;
...
/// [CusPopupMenuEntry.represents] method returns true for `initialValue`, and then
/// summing the values of [CusPopupMenuEntry.height] for all the preceding widgets
/// in the list.
///
...
///  * [PopupMenuDivider], a popup menu entry that is just a horizontal line.
///  * [CheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [CusPopupMenuButton], which provides an [IconButton] that shows a menu by
...
Future<T?> showMenu<T>({
  ...
  required List<CusPopupMenuEntry<T>> items,
  T? initialValue,
  double? elevation,
...
/// argument is the value of the [CusPopupMenuItem] that caused its menu to be
/// dismissed.
///
/// Used by [CusPopupMenuButton.onSelected].
typedef PopupMenuItemSelected<T> = void Function(T value);

/// Signature for the callback invoked when a [CusPopupMenuButton] is dismissed
/// without selecting an item.
///
/// Used by [CusPopupMenuButton.onCanceled].
typedef PopupMenuCanceled = void Function();

/// Signature used by [CusPopupMenuButton] to lazily construct the items shown when
/// the button is pressed.
///
/// Used by [CusPopupMenuButton.itemBuilder].
typedef PopupMenuItemBuilder<T> = List<CusPopupMenuEntry<T>> Function(
...
/// then [CusPopupMenuButton] behaves like an [IconButton].
///
...
/// // not shown here).
/// CusPopupMenuButton<WhyFarther>(
///   onSelected: (WhyFarther result) { setState(() { _selection = result; }); },
///   itemBuilder: (BuildContext context) => <CusPopupMenuEntry<WhyFarther>>[
///     const CusPopupMenuItem<WhyFarther>(
///       value: WhyFarther.harder,
///       child: Text('Working a lot harder'),
///     ),
...
///  * [CheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
class CusPopupMenuButton<T> extends StatefulWidget {
  /// Creates a button that shows a popup menu.
  ///
  /// The [itemBuilder] argument must not be null.
  const CusPopupMenuButton({
    super.key,
...
/// The [State] for a [CusPopupMenuButton].
///
/// See [showButtonMenu] for a way to programmatically open the popup menu
/// of your button state.
class PopupMenuButtonState<T> extends State<CusPopupMenuButton<T>> {
  /// A method to show a popup menu with the items supplied to
  /// [CusPopupMenuButton.itemBuilder] at the position of your [CusPopupMenuButton].
  ///
  /// By default, it is called when the user taps the button and [CusPopupMenuButton.enabled]
  ...
  void showButtonMenu() {
    ...
    final List<CusPopupMenuEntry<T>> items = widget.itemBuilder(context);
