...
class _FlutterReflectiveScreensaverState
  ...
  void addPosition() async {
    while (true) {
      ...
      if (location.dy >=
              (deviceLength?.height ?? 0) - (widgetSize?.height ?? 0) &&
          vector == vectors(3)) {
        vector = vectors(0);
      } else if (location.dy >=
              (deviceLength?.height ?? 0) - (widgetSize?.height ?? 0) &&
      ...
      if (location.dx >=
              (deviceLength?.width ?? 0) - (widgetSize?.width ?? 0) &&
          vector == vectors(0)) {
        vector = vectors(1);
      } else if (location.dx >=
              (deviceLength?.width ?? 0) - (widgetSize?.width ?? 0) &&
  ...
  void didChangeDependencies() async {
    ...
    var randomWidth = (deviceLength?.width ?? 0) * 0.1 * Random().nextDouble() +
        (deviceLength?.width ?? 0) * 0.6;
    var randomHeight = (deviceLength?.height ?? 0) * 0.1 +
        Random().nextDouble() * (deviceLength?.height ?? 0) * 0.6;
